            ...
            ->schema([
                Forms\Components\Section::make('Participante y Proyecto')
                    ->schema([
                        ...
                        Forms\Components\Select::make('participant_type')
                            ...
                            ->default('student')
                            ->required(),
                        Forms\Components\Select::make('participant_role')->label('Rol')->options([
                        ...
                        Forms\Components\Toggle::make('is_minor')->label('Menor de edad'),
                        Forms\Components\Select::make('training_branch')->label('Rama formativa')->options(['SOCIO_SANITARIA' => 'Socio-sanitaria', 'TECNOLOGICA' => 'Tecnológica', 'ADMINISTRATIVA' => 'Administrativa', 'OTRA' => 'Otra']),
                    ])->columns(3),
                ...
                Forms\Components\Section::make('Fechas y Destino de la Estancia')
                    ->schema([
                        ...
                        Forms\Components\Toggle::make('fewer_opportunities')
                            ...
                            ->default(false),
                        Forms\Components\Textarea::make('fewer_opportunities_reason')->label('Motivo de menos oportunidades')->visible(fn (Get $get) => (bool) $get('fewer_opportunities')),
                    ])->columns(3),
                ...
                Forms\Components\Section::make('Datos bancarios y fianza')
                    ->schema([
                        ...
                        Forms\Components\Repeater::make('bond')->relationship()->schema([
                            Forms\Components\TextInput::make('amount')->label('Importe')->numeric()->default(275)->required(),
                            Forms\Components\TextInput::make('concept')->label('Concepto')->required(),
                            Forms\Components\TextInput::make('destination_iban')->label('Destino STEPV')->default(MobilityBond::DESTINATION_IBAN)->required(),
                            Forms\Components\Select::make('status')->label('Estado')->options(['pending' => 'Pendiente', 'validated' => 'Validada', 'rejected' => 'Rechazada'])->required(),
                            Forms\Components\Textarea::make('observations')->label('Observaciones'),
                        ])->maxItems(1)->columns(3),
                    ])->columns(2),
                Forms\Components\Section::make('Seguimiento y renuncia')
                    ->schema([
                        Forms\Components\Repeater::make('tutoringSessions')->relationship()->schema([
                            Forms\Components\Select::make('tutor_scope')->label('Tutoría')->options(['origin' => 'Origen', 'stay' => 'Estancia', 'company' => 'Empresa'])->required(),
                            Forms\Components\Select::make('mode')->label('Modalidad')->options(['in_person' => 'Presencial', 'online' => 'En línea'])->required(),
                            Forms\Components\DateTimePicker::make('held_at')->label('Fecha')->required(),
                            Forms\Components\Textarea::make('content')->label('Contenido')->required(),
                        ])->columns(2),
                        Forms\Components\Repeater::make('withdrawal')->relationship()->schema([
                            Forms\Components\DatePicker::make('occurred_at')->label('Fecha')->required(),
                            Forms\Components\Toggle::make('after_expenses')->label('Tras incurrir en gastos'),
                            Forms\Components\Toggle::make('force_majeure')->label('Fuerza mayor'),
                            Forms\Components\TextInput::make('reimbursable_expenses')->label('Gastos reembolsables')->numeric()->default(0),
                            Forms\Components\TextInput::make('funds_to_return')->label('Fondos a devolver')->numeric()->default(0),
                            Forms\Components\Textarea::make('notes')->label('Observaciones'),
                        ])->maxItems(1)->columns(2),
                    ]),
            ]);
    }
